...
const Thumbnail = () => {
    useEffect(() => {
        AOS.init({
            duration: 1200,  // Duration of the animation for Featuring
            easing: 'ease-in-out', // Add easing for smoother animations
            once: true,     // Ensure animations happen only once for better performance
        });
        AOS.refresh(); // Refresh AOS to handle potential issues with dynamic content
        AOS.refreshHard(); // Hard refresh to ensure all animations are applied
    }, []);
    ...
    return (
        <div className="w-full bg-black px-4 md:px-8 lg:px-20 py-10">
            ...
            <div data-aos="zoom-in" className="heading w-full flex justify-center items-center text-white mt-16 mb-12">
                <h1 className="mr-2 text-3xl md:text-4xl font-medium font-serif">Featured</h1>
